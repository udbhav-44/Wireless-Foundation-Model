...
def parse_args():
    parser = argparse.ArgumentParser(
        description="End-to-end pretraining for CA + LWM with torch-native patching."
    )
    parser.add_argument(
        "--scenarios",
        nargs="+",
    ...
    parser.add_argument(
        "--tb-logdir",
        type=str,
        default="runs/lwm_ca_pretraining",
        help="TensorBoard log directory.",
    )
    parser.add_argument(
        "--scheduler-step-per-batch", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--dataset-folder",
        type=str,
    ...
    parser.add_argument("--save-path", type=str, default="lwm_ca/model_weights_ca_e2e.pth")
...
def main():
    ...
    model = LWMWithPrepatchCA(gen_raw=False, snr_db=args.snr_db).to(device)
    ...
    if log_file is None:
        base_path = args.save_path or "pretraining_e2e"
